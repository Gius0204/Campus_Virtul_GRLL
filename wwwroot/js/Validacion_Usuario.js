 window.addEventListener('load', () => {
    ...
    if (contrasena) {
        contrasena.addEventListener('blur', () => {
            ...
            if (valor === '') return;
            if (valor.length <= LIMITES.CONTRASENA_MAX) {
                validaOK(contrasena);
            } else {
        ...
        contrasena.addEventListener('focus', () => {
            const inputBox = contrasena.closest('.input-box') || contrasena.parentElement;
            const aviso = inputBox.querySelector('.aviso');
            if (aviso) aviso.remove();
...
document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape' && modalRecuperacion && modalRecuperacion.classList.contains('active')) {
        modalRecuperacion.classList.remove('active');
        formRecuperacion.reset();
